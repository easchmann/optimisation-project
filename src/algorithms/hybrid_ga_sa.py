...
def run(
    ...
    for gen in range(n_gen):
        ...
        for idx in sorted_indices[:top_n]:
            ...
            # SA is stochastic local search and can end up worse than where it
            # started; only keep the refinement if it didn't regress fitness,
            # otherwise elitism's monotonic-improvement guarantee is broken.
            if improved_ind.fitness.values[0] <= population[idx].fitness.values[0]:
