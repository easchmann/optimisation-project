...
Compatible with the existing ACO, GA, and SA implementations.
"""
...
# Reuse GA's DEAP setup (assuming it's already imported/defined)
...
def run_hybrid_GA_SA(
    ...
    Parameters use GA's DEFAULT_PARAMS plus additional SA params:
        - sa_steps: number of SA moves per refinement (default: 50 * n)
        - sa_T0: initial temperature for SA (default: 10.0)
        - sa_gamma: cooling rate for SA (default: 0.95)
        - sa_refine_fraction: fraction of population to refine (default: 0.1)
    """
    # Merge parameters (GA params + SA params)
    from genetic_algorithm import DEFAULT_PARAMS as GA_DEFAULT_PARAMS
    
    SA_DEFAULT_PARAMS = {
        "sa_steps": None,  # resolved to 50*n at runtime
        "sa_T0": 10.0,
        "sa_gamma": 0.95,
        "sa_refine_fraction": 0.1,
    }
    
    p = {**GA_DEFAULT_PARAMS, **SA_DEFAULT_PARAMS, **params}
    ...
    for gen in range(n_gen):
        # --- GA Selection and Variation (same as original) ---
        if elitism:
        ...
        for idx in sorted_indices[:top_n]:
            ...
            improved_np, new_fitness = _sa_refine_solution(
    ...
    # --- Return best solution (same as GA) ---
